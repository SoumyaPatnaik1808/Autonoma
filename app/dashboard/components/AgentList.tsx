...
export default function AgentList({ refreshTrigger, onCreateClick }: AgentListProps) {
        ...
        return (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-800 bg-zinc-900/30 p-12 text-center">
                ...
                <button
                    ...
                    className="flex items-center gap-2 rounded-full bg-white px-6 py-2.5 text-sm font-bold text-black transition-transform hover:scale-105"
                >
                    <Plus size={18} />
                    Create New Agent
                </button>
    ...
    return (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <button
                onClick={onCreateClick}
                className="group flex h-full min-h-[180px] flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-700 bg-zinc-900/20 p-6 transition-colors hover:border-purple-500/50 hover:bg-zinc-900/50"
            >
                <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-zinc-800 text-zinc-400 group-hover:bg-purple-500/10 group-hover:text-purple-400 transition-colors">
                    <Plus size={24} />
                </div>
                <span className="font-medium text-zinc-400 group-hover:text-purple-300">New Agent</span>
            </button>
            ...
            {agents.map((agent) => (
                <div
                    key={agent.id}
                    className="group relative overflow-hidden rounded-2xl border border-white/5 bg-zinc-900/50 p-6 transition-all hover:border-purple-500/30 hover:bg-zinc-900/80"
                >
                    ...
                    <div className="relative z-10">
                        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-zinc-800 to-zinc-900 text-white border border-white/5 shadow-inner">
                            <Bot size={24} />
                        </div>
